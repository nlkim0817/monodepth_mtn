...
class Resnet18_md_v2(nn.Module):
    def __init__(self, num_in_layers):
        super(Resnet18_md_v2, self).__init__()
        # encoder
        self.conv1 = conv(num_in_layers, 64, 7, 2)  # H/2  -   64D
        self.pool1 = maxpool(3)  # H/4  -   64D
        self.conv2 = resblock_basic(64, 64, 2, 2)  # H/8  -  64D
        ...
        self.upconv2 = upconv(64, 32, 3, 2)
        self.iconv2 = conv(64+32+2+2, 32, 3, 1)
        self.disp2_layer = get_disp(32)

        self.upconv1 = upconv(32, 16, 3, 2)
        self.iconv1 = conv(16+2+2+2, 16, 3, 1)
        self.disp1_layer = get_disp(16)
    ...
    def forward(self, x):
        ...
        _udisp4_x2 = nn.functional.interpolate(self.udisp4, scale_factor=2, mode='bilinear', align_corners=True)

        upconv2 = self.upconv2(iconv3)
        concat2 = torch.cat((upconv2, skip1, self.udisp3, _udisp4_x2), 1)
        iconv2 = self.iconv2(concat2)
        self.disp2 = self.disp2_layer(iconv2)
        self.udisp2 = nn.functional.interpolate(self.disp2, scale_factor=2, mode='bilinear', align_corners=True)

        _udisp4_x4 = nn.functional.interpolate(_udisp4_x2, scale_factor=2, mode='bilinear', align_corners=True)
        _udisp3_x2 = nn.functional.interpolate(self.udisp3, scale_factor=2, mode='bilinear', align_corners=True)

        upconv1 = self.upconv1(iconv2)
        concat1 = torch.cat((upconv1, self.udisp2, _udisp3_x2, _udisp4_x4), 1)
        iconv1 = self.iconv1(concat1)
        self.disp1 = self.disp1_layer(iconv1)
        return self.disp1, self.disp2, self.disp3, self.disp4
