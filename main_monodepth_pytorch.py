    ...
    def viz(self):
        ...
        with torch.no_grad():
            for (i, data) in enumerate(self.loader):
                ...
                disp     = disp * disp_pp.shape[1];
                disp_pp  = disp_pp * disp_pp.shape[1];
                depth    = disp2disp( disp, 0.1*disp_pp.shape[1] );
                depth_pp = disp2disp( disp_pp, 0.1*disp_pp.shape[1] );
                left     = left[0].permute((1,2,0)).cpu().numpy() * 255.;
                left     = left[:,:,[2,1,0]].astype(np.uint8);
